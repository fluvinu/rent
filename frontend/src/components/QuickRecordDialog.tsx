...
export function QuickRecordDialog({
  ...
  const inferType = (value: string): { type: FieldType; parsedValue: any } => {
    ...
    if (!isNaN(Number(value)) && value.trim() !== "") return { type: "NUMBER", parsedValue: Number(value) };
    if (/^\d{4}-\d{2}-\d{2}$/.test(value.trim())) return { type: "DATE", parsedValue: value.trim() };
    return { type: "TEXT", parsedValue: value };
  };

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedTypeId && !newTypeName) return toast.error("Please select or create a dataset");
    ...
    try {
      if (selectedType) {
        ...
        for (const f of selectedType.fields) {
          if (f.required && (existingData[f.name] === undefined || existingData[f.name] === "" || existingData[f.name] === null)) {
  ...
  const exactMatch = types.find(t => t.name.toLowerCase() === search.trim().toLowerCase());
      ...
      <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-6">
            ...
            <Popover open={openCombobox} onOpenChange={setOpenCombobox}>
              <PopoverTrigger asChild>
                <Button variant="outline" role="combobox" aria-expanded={openCombobox} className="w-full justify-between">
                  {selectedTypeId ? types.find((t) => t.id === selectedTypeId)?.name : newTypeName ? `Create "${newTypeName}"` : "Select dataset..."}
                  <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                </Button>
              </PopoverTrigger>
              <PopoverContent className="w-[460px] p-0" align="start">
                <Command>
                  <CommandInput placeholder="Search or type new dataset..." value={search} onValueChange={setSearch} />
                  <CommandList>
                    <CommandEmpty>
                      {search.trim() ? (
                        <Button variant="ghost" className="w-full justify-start px-2 py-1.5" onClick={() => {
          ...
          {selectedType && (
            <div className="space-y-4 border-t pt-4">
              ...
              {selectedType.fields.map((f) => (
                <FieldInput key={f.name} field={f} value={existingData[f.name]} onChange={(v) => setExistingData(d => ({ ...d, [f.name]: v }))} />
              ))}
            </div>
          )}
               ...
               {kvPairs.map((kv, i) => (
                 <div key={i} className="flex items-center gap-2">
                   <div className="flex-1">
                     <Input placeholder="Field Name" value={kv.key} onChange={e => setKvPairs(p => p.map((x, idx) => idx === i ? { ...x, key: e.target.value } : x))} />
                   </div>
                   <div className="flex-1">
                     <Input placeholder="Value (e.g. true, 123, text)" value={kv.value} onChange={e => setKvPairs(p => p.map((x, idx) => idx === i ? { ...x, value: e.target.value } : x))} />
                   </div>
                   <Button type="button" variant="ghost" size="icon" onClick={() => setKvPairs(p => p.filter((_, idx) => idx !== i))} disabled={kvPairs.length === 1}>
                     <Trash2 className="size-4" />
                   </Button>
                 </div>
               ))}
               <Button type="button" variant="outline" size="sm" onClick={() => setKvPairs(p => [...p, { key: "", value: "" }])}>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={saving || (!selectedTypeId && !newTypeName)}>{saving ? "Saving..." : "Save Record"}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
